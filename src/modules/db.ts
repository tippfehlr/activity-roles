    ...
    this_role: {
      ...
      if (userShouldHaveRole && !userHasRole) { // add role to member
        if (role.position < highestBotRole) {
          ...
          msg.log.addedRoleToMember(role.name, guildActivity.roleID, member.user.username, member.user.id, member.guild.name, member.guild.id);
        ...
        if (role.position < highestBotRole) {
          ...
          msg.log.removedRoleFromMember(role.name, guildActivity.roleID, member.user.username, member.user.id, member.guild.name, member.guild.id);
        } else if ('logChannelID' in await GuildConfig.findById(member.guild.id.toString()).lean()) {
          const _guildConfig = await GuildConfig.findById(member.guild.id.toString()).lean();
          const channel = member.guild.channels.cache.find(_channel => _channel.id === _guildConfig.logChannelID && _channel.isText()) as Discord.TextChannel;
          if (channel) {
          ...
          if (userShouldHaveRole && !userHasRole) { // add role to member
            if (role.position < highestBotRole) {
              ...
              if (channel) {
                ...
                channel.send({ embeds: [msg.errorCantAssignRole(role.id, role.position, member.user.id, guildActivityList[x].activityName, highestBotRole)] });
              }
            }
          } else if (!userShouldHaveRole && userHasRole) { // remove role from member
            if (role.position < highestBotRole) {
              ...
              if (channel) {
                ...
                channel.send({ embeds: [msg.errorCantRemoveRole(role.id, role.position, member.user.id, guildActivityList[x].activityName, highestBotRole)] });
              }
            }
          }
        }
