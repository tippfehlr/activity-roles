	...
	data: new SlashCommandBuilder()
		...
		.addBooleanOption(option =>
			option
				...
				.setDescriptionLocalizations(
					discordTranslations('deleteActivityRole->description->all'),
				)
				.setRequired(false),
		),
	...
	execute: async interaction => {
		...
		if (all) {
			...
			return interaction.reply({
				content: __({
					phrase: 'You need to provide a role or an activity. If you provide a role, all activity roles for that role will be deleted. If you provide an activity, all activity roles with that name will be deleted.\nIf you provide both, all activity roles with that name for that role will be deleted.',
					locale,
				}),
				flags: MessageFlags.Ephemeral,
			});
